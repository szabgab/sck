...
any ['get', 'post'] => '/' => sub {
    ...
    my $url = Celogeek::URL->new('redis' => redis, max_generated_times => 5, max_letters => $max_letters);
    my $shorturl = "";
    my $statsurl = "";
    my $warning  = "";
    my $top10 = [];
    ...
    if ($longurl) {
        ...
        if ($short eq 'NO WAY TO SHORTEN') {
            ...
            $warning .= "Your url is bad. It has to start with 'http://' or 'https://'."
        } elsif ($short eq 'TOO MANY TRIES') {
            $warning .= "Too many tries (> ".$url->max_generated_times."). Try again.";
        } else {
            $shorturl = $base.$short;
            $statsurl = $shorturl."?s=1";
            if ($url->generated_times) {
                $warning .= "Generated after ".$url->generated_times." tries.";
            } else {
