...
sub BUILD {
    ...
    $self->redis->incr('c:min_letters')
      unless $self->redis->get('c:min_letters');
}
...
sub stats {
    ...
    if ( $self->redis->exists( $self->_path_key($key) ) ) {
        ...
        foreach my $d (qw/created_at last_accessed_at/) {
            if ( $data->{$d} ) {
                $data->{$d} =
                  $self->_datetime( $data->{$d} )->strftime( $opts->{date} );
            }
        }
        return $data;
    }
...
sub _get {
    ...
    my $hash_key = shift;
    my %data =
      $self->redis->exists($hash_key) ? $self->redis->hgetall($hash_key) : ();
    return \%data;
}
...
sub top10 {
    ...
    for ( my $i = 0 ; $i < @members ; $i += 2 ) {
        ...
        if ( $data->{title} ) {
            ...
            if ( ( $self->redis->hget( $hash_key, 'missing_title_tries' ) || 0 )
                < 5 )
            {
